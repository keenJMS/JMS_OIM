...
from .util import ship_data_to_cuda
...
def inference(model, gallery_loader, probe_loader, device):
    ...
    for (images,targets) in tqdm(gallery_loader, ncols=0):
        ...
        images, targets = ship_data_to_cuda(images,targets, device)
        ...
        for o, t in zip(outputs, targets):
            ...
            all_feats.append(o['person_feat'].to(cpu).numpy())

    probe_feats = []
    for (images,targets) in tqdm(probe_loader, ncols=0):
        images, targets = ship_data_to_cuda(images,targets, device)
        person_feats = model.ex_feat(images, targets, mode='det')
        for pf in person_feats:
            probe_feats.append(pf.to(cpu).numpy())
...
def detection_performance_calc(dataset, gallery_det, det_thresh=0.5, iou_thresh=0.5,
                               labeled_only=False,logger=None):
    """
